...
Để giải quyết các bài toán đặt ra trong bối cảnh trên, đồ án xác định mục tiêu tổng quát là thiết kế, chế tạo và tích hợp hoàn chỉnh một hệ thống IoT quản lý Smart Building dựa trên chuẩn truyền thông không dây Zigbee, kết nối qua Gateway C tới hệ thống Cloud Backend, hỗ trợ giám sát trạng thái thiết bị thực tế gần thời gian thực và tự động hóa điều khiển phụ tải. Để hiện thực hóa mục tiêu đó, đồ án đề ra các mục tiêu cụ thể cho từng thành phần trong hệ thống:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.1cm]
        % --- Các khối giai đoạn ---
        \node[fblk, cCloud, minimum width=10cm]
            (gd1) {\textbf{Giai đoạn 1:} Khảo sát và nghiên cứu công nghệ\\
                   {\scriptsize Zigbee 3.0, ZCL, MQTT, REST API}};

        \node[fblk, cGw, minimum width=10cm, below=of gd1]
            (gd2) {\textbf{Giai đoạn 2:} Thiết kế hệ thống\\
                   {\scriptsize Kiến trúc 4 lớp, CSDL PostgreSQL, MQTT Contract}};

        \node[fblk, cMob, minimum width=10cm, below=of gd2]
            (gd3) {\textbf{Giai đoạn 3:} Triển khai chi tiết từng module\\
                   {\scriptsize Firmware EFR32, Z3Gateway (C), FastAPI Cloud}};

        \node[fblk, cQueue, minimum width=10cm, below=of gd3]
            (gd4) {\textbf{Giai đoạn 4:} Tích hợp hệ thống\\
                   {\scriptsize NCP--UART--Gateway--Mosquitto--Cloud}};

        \node[fblk, cZig, minimum width=10cm, below=of gd4]
            (gd5) {\textbf{Giai đoạn 5:} Kiểm thử và đánh giá\\
                   {\scriptsize pytest, kiểm thử E2E trên phần cứng thật}};

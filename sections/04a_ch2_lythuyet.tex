...
\vspace{0.5cm}
\noindent\textit{Chương 2 trình bày cơ sở lý thuyết và công nghệ dùng để xây dựng hệ thống Internet of Things (IoT) cho Smart Building. Nội dung gồm khái niệm tòa nhà thông minh, mạng Zigbee, mô hình thiết bị theo Zigbee Cluster Library, vai trò của Gateway, giao thức MQTT, giao tiếp giữa Host và EFR32 NCP, Cloud Backend, ứng dụng di động và các nguyên tắc bảo mật cơ bản. Chương chỉ trình bày kiến thức nền, không đi vào mã nguồn hoặc kết quả thực nghiệm.}
\vspace{0.5cm}

% ========================================
\section{Khái niệm Smart Building và IoT}
\label{sec:ch2-iot-smart-building}

Smart Building, hay tòa nhà thông minh, là công trình sử dụng cảm biến, thiết bị chấp hành, mạng truyền thông và phần mềm để giám sát hoặc điều khiển các chức năng trong tòa nhà. Mục tiêu không chỉ là điều khiển từ xa mà còn là tạo vòng phản hồi: thu thập trạng thái, xử lý dữ liệu, thực hiện hành động và ghi nhận kết quả. Trong đề tài này, phạm vi Smart Building được thu nhỏ thành mô hình một phòng thông minh có đèn, công tắc, cảm biến chuyển động, Gateway, Cloud Backend và ứng dụng di động.

Internet of Things (IoT), hay Internet vạn vật, là mô hình kết nối thiết bị vật lý với phần mềm thông qua mạng truyền thông. Thiết bị trong hệ thống IoT thường có cảm biến, bộ xử lý, phần mềm nhúng và khả năng trao đổi dữ liệu. Cảm biến thu thập trạng thái, thiết bị chấp hành thực hiện hành động vật lý, Gateway nối mạng thiết bị cục bộ với hệ thống phía trên, còn Cloud Backend lưu dữ liệu và cung cấp Application Programming Interface (API) cho ứng dụng.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{Images/mermaid/ch2_iot_architecture.png}
...
Để lựa chọn công nghệ truyền thông cho lớp thiết bị cục bộ, cần so sánh Zigbee với một số công nghệ IoT liên quan theo phạm vi của đồ án. Bảng \ref{tab:ch2-iot-protocol-comparison} không nhằm kết luận công nghệ nào tốt nhất trong mọi trường hợp, mà làm rõ công nghệ nào phù hợp hơn với mô hình phòng/lab gồm cảm biến, công tắc, đèn, Gateway và Cloud/App.

\begin{table}[H]
\centering
\caption{So sánh một số công nghệ IoT theo phạm vi đồ án}
\label{tab:ch2-iot-protocol-comparison}
\small
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{4.8cm}|>{\raggedright\arraybackslash}p{5.2cm}|}
\hline
\textbf{Công nghệ} & \textbf{Điểm mạnh chính} & \textbf{Đánh giá với phạm vi đồ án} \\ \hline
Zigbee & Công suất thấp, hỗ trợ mesh, có mô hình thiết bị và hệ sinh thái Smart Home/Building đã phổ biến \cite{csa-zigbee-page}. & Phù hợp với các node light, switch và motion sensor truyền trạng thái hoặc lệnh nhỏ trong mạng cục bộ. Cần Gateway để nối lên Cloud/App. \\ \hline
Wi-Fi và Wi-Fi HaLow & Wi-Fi thuận tiện khi thiết bị cần kết nối IP trực tiếp; Wi-Fi HaLow được định hướng cho ứng dụng IoT công suất thấp và phạm vi rộng hơn Wi-Fi truyền thống \cite{wifi-halow}. & Phù hợp hơn với thiết bị cần băng thông hoặc IP trực tiếp. Với sensor/actuator nhỏ trong đồ án, dùng Wi-Fi cho từng node có thể làm tăng yêu cầu năng lượng và cấu hình mạng. \\ \hline
Thread & Thread là mạng mesh công suất thấp dựa trên Internet Protocol (IP), sử dụng IEEE 802.15.4 và không phụ thuộc vào một application layer cố định \cite{thread-benefits}. & Có hướng phát triển tốt cho hệ sinh thái IP-based, nhưng đồ án hiện đã có thiết bị Zigbee, Zigbee Cluster Library và Z3Gateway C; chuyển sang Thread sẽ làm thay đổi stack thiết bị và Gateway. \\ \hline
Long Range Wide Area Network (LoRaWAN) & LoRaWAN là giao thức Low Power Wide Area Network (LPWAN), phù hợp kết nối thiết bị dùng pin ở phạm vi xa \cite{lorawan-spec}. & Phù hợp hơn với bài toán cảm biến phân tán ngoài trời hoặc nhiều khu vực xa nhau. Phạm vi phòng/lab của đồ án không cần đánh đổi độ trễ và băng thông thấp theo hướng LPWAN. \\ \hline
Matter & Matter là giao thức Smart Home mở, chạy trên các nền mạng như Wi-Fi, Thread và Ethernet để tăng khả năng liên thông giữa hệ sinh thái \cite{matter-faq}. & Matter có thể là hướng mở rộng interoperability sau này, nhưng không thay thế trực tiếp lớp Zigbee runtime hiện tại của đồ án. \\ \hline
\end{tabular}
\end{table}

Từ bảng so sánh, Zigbee được chọn không phải vì luôn vượt trội hơn mọi giao thức IoT khác, mà vì khớp với bài toán cụ thể: thiết bị công suất thấp, bản tin nhỏ, có thể mở rộng mesh và có mô hình chức năng thiết bị rõ ràng qua ZCL. Cách lựa chọn này cũng giữ kiến trúc báo cáo nhất quán với triển khai thực tế: thiết bị Zigbee giao tiếp với Gateway, Gateway chuyển đổi sang MQTT, còn Cloud/App không cần xử lý trực tiếp chi tiết mạng Zigbee.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.68\textwidth]{Images/mermaid/ch2_mesh_topology.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.66\textwidth]{Images/mermaid/ch2_zigbee_stack.png}
...
Trong đồ án này, Gateway được hiểu theo kiến trúc hiện tại: Z3Gateway C Host App kết nối với EFR32 NCP và tích hợp MQTT trực tiếp thông qua \texttt{libmosquitto}. Cloud và Mobile App trao đổi dữ liệu qua MQTT Broker; Z3Gateway C nhận command từ MQTT, gọi Zigbee action qua Ember AF/EZSP, sau đó publish reported state hoặc event trở lại MQTT khi thiết bị phản hồi.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{Images/mermaid/ch2_mqtt_publish_subscribe.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.68\textwidth]{Images/mermaid/ch2_host_ncp_uart.png}
...
% ========================================
\section{Ứng dụng di động và quản lý trạng thái}
...
% ========================================
\section{Bảo mật trong Zigbee và IoT}
\label{sec:ch2-security}

Bảo mật cần được xem xét ở nhiều lớp, từ thiết bị, mạng Zigbee, Gateway, MQTT Broker, Cloud Backend, cơ sở dữ liệu đến ứng dụng. Với Smart Building, sự cố bảo mật có thể dẫn đến mất dữ liệu hoặc làm sai lệch hành vi điều khiển thiết bị vật lý. Vì vậy, thiết kế phải kiểm soát cả thiết bị tham gia mạng, kênh truyền và quyền của người dùng.
...
Ở lớp MQTT và Cloud, broker cần xác thực client, giới hạn quyền trên topic và bảo vệ kênh truyền bằng Transport Layer Security (TLS). Cloud Backend cần xác thực người dùng, kiểm tra vai trò, giới hạn phạm vi nhà/thiết bị, kiểm tra dữ liệu đầu vào và bảo vệ cơ sở dữ liệu. Ứng dụng cũng cần lưu token trong vùng lưu trữ bảo mật thay vì tệp cấu hình thông thường.

Tăng cường bảo mật làm quá trình triển khai, vận hành và gỡ lỗi phức tạp hơn. Sự đánh đổi này cần được kiểm soát bằng cấu hình môi trường, test quyền truy cập và log có định danh rõ ràng. Báo cáo chỉ khẳng định các cơ chế đã có source và test tương ứng.
